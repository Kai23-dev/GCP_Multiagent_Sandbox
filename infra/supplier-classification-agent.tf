module "supplier_classification_agent" {
  ...
  agent_id   = "supplier-classification-agent"
  ...
  # Agent deployment
  agent_source_path               = "./agents/supplier_classification_agent"
...
resource "google_project_iam_member" "supplier_classification_agent_access" {
  ...
  member = "serviceAccount:${module.supplier_classification_agent.service_account_email}"
}

resource "google_project_iam_member" "supplier_classification_agent_bq_job_user" {
  project = local.join_sco_kb.project_id
  role    = "roles/bigquery.jobUser"
  member  = "serviceAccount:${module.supplier_classification_agent.service_account_email}"
}

resource "google_project_iam_member" "supplier_classification_agent_bq_data_viewer" {
  project = local.join_sco_kb.project_id
