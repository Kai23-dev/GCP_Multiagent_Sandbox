module "visualization_agent" {
  ...
  additional_env_vars = {
    ...
    VISUALIZATION_BUCKET                               = module.agent_project.visualizations_bucket
  ...
  # Agent deployment
  agent_source_path = "./agents/visualization_agent"
...
resource "google_project_iam_member" "visualization_agent_access" {
  ...
  member = "serviceAccount:${module.visualization_agent.service_account_email}"
}

resource "google_project_iam_member" "visualization_agent_bq_job_user" {
  project = local.join_sco_kb.project_id
  role    = "roles/bigquery.jobUser"
  member  = "serviceAccount:${module.visualization_agent.service_account_email}"
}

resource "google_storage_bucket_iam_member" "visualizations_agent_vis_writer" {
  bucket = module.agent_project.visualizations_bucket
