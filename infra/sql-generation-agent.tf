module "sql_generation_agent" {
  ...
  additional_env_vars = {
    ...
    TABLE_SCHEMA_GCS_PREFIX                            = "ai_financial_dlp/"
  }
  ...
  # Agent deployment
  agent_source_path = "./agents/sql_generation_agent"
  staging_bucket    = module.agent_project.agent_staging_bucket
}

resource "google_project_iam_member" "sql_generation_agent_access" {
  ...
  member = "serviceAccount:${module.sql_generation_agent.service_account_email}"
}

resource "google_project_iam_member" "sql_generation_agent_bq_job_user" {
  project = local.join_sco_kb.project_id
  role    = "roles/bigquery.jobUser"
  member  = "serviceAccount:${module.sql_generation_agent.service_account_email}"
}

resource "google_storage_bucket_iam_member" "sql_generation_agent_table_schemas_reader" {
