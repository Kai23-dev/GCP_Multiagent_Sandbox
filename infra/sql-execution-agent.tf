module "sql_execution_agent" {
  ...
  # Agent deployment
  agent_source_path = "./agents/sql_execution_agent"
  staging_bucket    = module.agent_project.agent_staging_bucket
  depends_on        = [module.sql_generation_agent]
}

resource "google_project_iam_member" "sql_execution_agent_access" {
  ...
  member = "serviceAccount:${module.sql_execution_agent.service_account_email}"
}

resource "google_project_iam_member" "sql_execution_agent_bq_job_user" {
